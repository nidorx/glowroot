...
angular
        ...
        .controller('TransactionTabTracesCtrl', TransactionTabTracesCtrl);


TransactionTabTracesCtrl.$inject = [
    '$scope', '$location', '$http', '$q', 'locationChanges', 'charts', 'httpErrors', 'traceModal', 'queryStrings', 'model'
];

function TransactionTabTracesCtrl($scope, $location, $http, $q, locationChanges, charts, httpErrors, traceModal, queryStrings, model) {
    ...
    $scope.traceKind = $scope.$state.is('page.transaction.detail.traces') ? 'transaction' : 'error';
    $scope.showErrorMessageFilter = $scope.traceKind === 'error';
    ...
    function refreshChart(autoRefresh, deferred) {
        ...
        if ($scope.model.transactionName) {
            query.transactionName = $scope.model.transactionName;
        }
        if (autoRefresh) {
    ...
    locationChanges.on($scope, function () {
        ...
        $scope.traceAttributeNames = $scope.layout.agentRollups[$scope.agentRollupId]
                .traceAttributeNames[$scope.model.transactionType];
